...
// ---------------------------------------------------------------------
// Customer withdrawals — manual review. SecureWaveNG's automated
// payout endpoint is not currently available for this merchant, so
// every withdrawal is created 'pending' by lib/withdrawalService.js
// (wallet already debited at request time) and an admin manually
// pays the customer's bank account, then calls one of these:
//
//   approve-withdrawal: the payout was sent — mark it done. No wallet
//     change (money left the wallet when the request was made).
//   reject-withdrawal: the payout was NOT sent — refund the amount +
//     fee back to the customer's wallet.
//
// Same double-processing guard as manual funding: only proceeds if
// the row's status is still 'pending' at UPDATE time.
// ---------------------------------------------------------------------
async function approveWithdrawal(req, res) {
  ...
  const { data: claimed, error } = await supabaseAdmin
    ...
    .update({ status: 'successful', reviewed_by: req.user.id, reviewed_at: new Date().toISOString() })
    .eq('id', requestId)
    .eq('status', 'pending')
    .select()
    .maybeSingle();
  ...
  if (claimed.transaction_id) {
    await supabaseAdmin.from('transactions').update({ status: 'success' }).eq('id', claimed.transaction_id);
  }

  return sendSuccess(res, { requestId: claimed.id }, { message: 'Withdrawal marked as paid' });
}
...
async function rejectWithdrawal(req, res) {
  ...
  const { data: claimed, error } = await supabaseAdmin
    ...
    .update({
      status: 'failed',
      error_message: adminNote || 'Rejected by admin',
  ...
  const refundAmount = Number(claimed.amount) + Number(claimed.fee || 0);

  await wallet.refundDebit({
    userId: claimed.user_id,
    amount: refundAmount,
    originalReference: claimed.wallet_debit_reference,
    description: `Refund: withdrawal request rejected by admin${adminNote ? ` (${adminNote})` : ''}`
  });

  if (claimed.transaction_id) {
    await supabaseAdmin.from('transactions').update({ status: 'failed' }).eq('id', claimed.transaction_id);
  }

  return sendSuccess(res, { requestId: claimed.id, refundedAmount: refundAmount }, { message: 'Withdrawal rejected and refunded' });
}
