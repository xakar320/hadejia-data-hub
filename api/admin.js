...
async function testProvider(req, res) {
  ...
  throw new ValidationError('Unknown provider test', {
    details: [{ field: 'provider', message: 'provider must be one of: supercheapdata-airtime' }]
  });
}
...
async function testSuperCheapDataAirtime(req, res, body) {
  ...
  if (!apiKey) {
    throw new Error('SUPERCHEAPDATA_API_KEY is not set in Vercel environment variables yet.');
  }
