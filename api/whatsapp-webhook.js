...
async function myAccount(session) {
  const { data: user, error } = await supabaseAdmin
    .from('users')
    .select('full_name, phone, email, wallet_balance, is_reseller')
    .eq('id', session.user_id)
    .maybeSingle();
  ...
  let resellerLine;
  if (user.is_reseller) {
    resellerLine = '\n\n✅ You are an approved Reseller — reseller pricing applies automatically.';
  } else {
    ...
    resellerLine = pendingApp
      ? '\n\n⏳ Your reseller application is under review.'
      : '\n\nWant to sell at reseller prices? Reply *RESELLER* to apply.';
  }
...
async function applyForResellerWhatsApp(session) {
  const { data: user } = await supabaseAdmin.from('users').select('is_reseller').eq('id', session.user_id).maybeSingle();

  if (user && user.is_reseller) {
    await goToMainMenu(session, "You're already an approved Reseller — no need to apply again.");
    return;
  }
  ...
  if (pendingApp) {
    await goToMainMenu(session, '⏳ You already have a reseller application under review.');
    return;
  }

  const { error } = await supabaseAdmin.from('reseller_applications').insert({ user_id: session.user_id });

  if (error) {
    console.error('[applyForResellerWhatsApp] Failed to create application:', error.message);
    await goToMainMenu(session, '⚠️ Sorry, something went wrong submitting your application. Please try again.');
    return;
  }

  await goToMainMenu(session, '✅ Application submitted! We\'ll review it and let you know.');
}
...
async function handleDataPhone(session, text) {
  ...
  const phone = validation.normalizeNigerianPhone(text.trim());
  if (!phone) {
...
async function handleIncomingMessage(waNumberRaw, text) {
  ...
  // ---- Global RESELLER command ------------------------------------------
  if (/^reseller$/i.test(trimmed)) {
    if (!session.user_id) {
      await sendMessage(session.wa_number, 'Please create an account first — send *Hi* to get started.');
      return;
    }
    await applyForResellerWhatsApp(session);
    return;
  }
