...
/**
 * Look up a provider/network-level product_id from ANY active plan row
 * matching that provider — used when there's no specific planId (e.g.
 * cable "renew", where the amount is the customer's own outstanding
 * balance rather than a fixed plan). product_id is shared across every
 * package for a given provider, so any one configured row will do.
 */
 ...
 * product_id is never taken from the client — it comes from the
 * airtime_plans row (set once per network by admin in admin.html),
 * same integrity guarantee as data/voice/electricity.
 */
...
/**
 * type: "cable"
 * body (change subscription): { cableType: "change", planId, iucNumber, isBoxOffice? }
 * body (renew / pay bill):    { cableType: "renew" (default), network, iucNumber, amount, isBoxOffice? }
 * product_id is never taken from the client in either mode.
 */
...
async function placeOrder(req, res) {
  ...
  validateRequiredFields(body, ['type', 'pin']);

  // Verified BEFORE anything else touches the wallet — a wrong PIN
  // must never get as far as debiting the wallet or calling AutosyncNG.
  const pin = validatePin(body.pin, 'pin');
