 ...
 * single-use, so the row is marked inactive immediately after use to
 * prevent a stale account_number match on a later, unrelated payment.
 *
...
async function webhookHandler(req, res) {
  ...
  // The HMAC must be computed over the EXACT raw bytes SecureWaveNG
  // signed, before JSON parsing. If Vercel's default body parser is
  // active on this route, req.rawBody won't exist and this falls back
  // to re-serializing req.body — which only matches the original bytes
  // if key order/whitespace happen to be identical, so it is NOT
  // reliable. Configure this route with a raw body parser
  // (e.g. `export const config = { api: { bodyParser: false } }` plus
  // manually reading the stream) before relying on this in production.
  ...
  // Idempotent on the account's own reference — a duplicate webhook
  // delivery for the same payment resolves to the same transaction
  // instead of crediting twice.
  const txn = await transactions.createPendingTransaction({
    userId: dynAccount.user_id,
    type: 'wallet_funding',
    amount: netAmount,
    idempotencyKey: dynAccount.provider_ref,
    requestPayload: {
  ...
  if (txn.status === 'pending') {
    await wallet.creditWallet({
      ...
      reference: dynAccount.provider_ref
    });
  ...
  // Single-use: deactivate immediately so a later, unrelated payment
  // notification can never match this same account_number again.
