...
async function webhookHandler(req, res) {
  ...
  // SecureWaveNG sends every notification (funding AND withdrawal) to
  // this same webhook URL. We don't have confirmed docs for the exact
  // shape of a withdrawal notification, so this tries every reference
  // field SecureWaveNG uses elsewhere (top-level `reference`, nested
  // `data.reference`, or `transaction_id`) against our own
  // withdrawal_requests.provider_reference. If one matches a
  // still-processing withdrawal, handle it here and stop — otherwise
  // fall through to the funding-account logic below unchanged.
  const possibleWithdrawalRef =
    payload.reference || (payload.data && payload.data.reference) || payload.transaction_id || null;

  if (possibleWithdrawalRef) {
    const rawStatus = String(
      payload.status || payload.transaction_status || payload.notification_status || ''
    ).toLowerCase();
    const isSuccess = SUCCESS_STATUSES.includes(rawStatus);
    const isFailure = ['failed', 'reversed', 'declined', 'cancelled'].includes(rawStatus);

    if (isSuccess || isFailure) {
      const { matched } = await withdrawalService.handleWebhookStatus({
        reference: possibleWithdrawalRef,
        success: isSuccess,
        message: payload.message || payload.narration || null
      });

      if (matched) {
        return sendSuccess(res, null, { message: `Withdrawal ${possibleWithdrawalRef} acknowledged (${rawStatus})` });
      }
    }
  }
