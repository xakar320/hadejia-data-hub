 ...
 * Talk More, Cable "Change Subscription", Betting, Internet, SMS, and
 * Data PIN endpoints, which follow the same request/response pattern
 * and are included in SERVICE_ENDPOINTS below for getCategories()/
 * getProducts() but are not wired to a dedicated purchase*() function
 * since none were requested.
 *
...
const SERVICE_ENDPOINTS = {
  ...
  'talk-more': { get: '/talk-more', post: '/talk-more' },
...
function normalizeResponse(response) {
  ...
  const txn = (body.data && body.data.transaction) || null;

  // Request-level error (top-level "status": "error") — no transaction object at all.
  if (body.status === 'error' || !txn) {
    return {
      success: false,
      status: 'error',
