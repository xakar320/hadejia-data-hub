...
const SERVICE_ENDPOINTS = {
  ...
  'data-gifting': { get: '/data', post: '/data' },
...
/**
 * A response is worth retrying only for READ-ONLY (GET) requests —
 * network failure, timeout, or a 5xx. GET requests have no side
 * effects, so retrying one is always safe.
 *
 * POST requests (purchases) are NEVER retried automatically, full
 * stop. The original design here assumed AutosyncNG's request_ref
 * would dedupe a retried purchase on their end — real-world evidence
 * showed that assumption is FALSE: a request that times out on our
 * side but actually reached and completed on AutosyncNG's side gets
 * processed a SECOND time on retry, causing a double charge and a
 * duplicate delivery. A timeout is fundamentally ambiguous — we
 * cannot know whether the request reached them — so for anything
 * that spends money, the only safe move is to not guess. If a
 * purchase call times out, it surfaces as a single 'failed'
 * transaction (refunded), and the reconciliation tooling
 * (admin.html's "Check real status" + the auto-reconcile cron job)
 * is what catches the case where it actually succeeded.
 */
 ...
 * transient failures — GET requests only (see isRetryable above).
 */
async function requestWithRetry(config, context) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt += 1) {
    try {
      ...
      if (isLastAttempt || !isRetryable(error, config.method)) {
        break;
      }
...
function normalizeFailure(error) {
  ...
  return {
    ...
    // We genuinely don't know what happened on AutosyncNG's side for
    // a timeout or network error — no response came back at all. This
    // is DIFFERENT from a definitive rejection (like "Invalid product
    // code"), where AutosyncNG DID respond and told us it failed.
    // Callers (see orderService.js#executeOrder) use this to avoid
    // treating "we don't know" the same as "it definitely failed".
    isTimeout: error.code === 'ECONNABORTED',
    isNetworkError: !error.response && !!error.request,
    data: txn,
    raw: body
...
/**
 * "Airtime Share & Sell" — a separate AutosyncNG product line from
 * regular purchaseAirtime() above (different product_id catalog,
 * different endpoint), but confirmed via live docs to use the exact
 * same request body shape and the exact same response envelope
 * (already handled by normalizeResponse's `data.transaction.status`
 * branch) — so this is a near copy of purchaseAirtime(), just
 * pointing at SERVICE_ENDPOINTS['airtime-share'].
 *
 * Maps to: POST /airtime/share
 *
 * @param {Object} params
 * @param {string} params.phone
 * @param {string} params.productId  - product id/code from getProducts('airtime-share'), NOT the same catalog as regular purchaseAirtime
 * @param {number} params.amount
 * @param {string} [params.webhookUrl]
 * @param {boolean} [params.portedNo]
 * @param {string} [params.reference]  - your request_ref; auto-generated if omitted
 */
async function purchaseAirtimeShare({ phone, productId, amount, webhookUrl, portedNo, reference }) {
  const requestRef = reference || generateRequestRef('AIRSH');

  return callProvider(
    {
      method: 'POST',
      url: SERVICE_ENDPOINTS['airtime-share'].post,
      data: {
        request_ref: requestRef,
        phone,
        product_id: productId,
    ...
    'purchaseAirtimeShare'
  );
}

/**
 * Send an SMS to one or more Nigerian numbers.
 * Maps to: POST /sms
 *
 * Pricing note (per docs): units = ceil(message.length / 160) x
 * recipients.length, billed per unit. This function does not compute
 * price — the caller (orderService.js#resolveSmsOrder) is responsible
 * for pricing against sms_plans before calling this.
 *
 * @param {Object} params
 * @param {string[]|string} params.recipients - array or comma-separated string of Nigerian MSISDNs (234XXXXXXXXXX or 0XXXXXXXXXX)
 * @param {string} params.message    - SMS body, max 1000 characters
 * @param {string} params.senderId   - approved sender ID, max 20 characters
 * @param {string} [params.webhookUrl]
 * @param {string} [params.reference] - your request_ref; auto-generated if omitted
 */
async function purchaseSms({ recipients, message, senderId, webhookUrl, reference }) {
  const requestRef = reference || generateRequestRef('SMS');
  const recipientsPayload = Array.isArray(recipients) ? recipients.join(',') : recipients;

  return callProvider(
    {
      method: 'POST',
      url: SERVICE_ENDPOINTS.sms.post,
      data: {
        request_ref: requestRef,
...
/**
 * Initiate an Airtime-to-Cash request — the FIRST of two steps.
 * Maps to: POST /airtime/cash
 *
 * This shares airtime FROM the sender's own phone (using their MTN
 * airtime-share PIN) into your AutosyncNG account. AutosyncNG sends
 * an OTP to that phone number to authorize the share; the resulting
 * txn.status is normally 'pending' at this point (NOT an error) —
 * you must call completeAirtimeToCash() with the OTP next.
 *
 * Amount limits/splitting and daily caps are enforced by AutosyncNG
 * itself, not here.
 *
 * @param {Object} params
 * @param {string} params.phone      - sender's MTN phone number holding the airtime
 * @param {string} params.productId  - product id/code (e.g. 'mtn')
 * @param {number} params.amount     - airtime value to collect
 * @param {string} params.sharePin   - the sender's MTN airtime-share PIN (never store this)
 * @param {string} [params.reference] - your request_ref; auto-generated if omitted
 */
async function initiateAirtimeToCash({ phone, productId, amount, sharePin, reference }) {
  const requestRef = reference || generateRequestRef('ATC');
  ...
  return callProvider(
    ...
    'initiateAirtimeToCash'
  );
}

/**
 * Complete an Airtime-to-Cash request — the SECOND step. Confirms
 * the OTP sent to the sender's phone to actually move the airtime.
 * Maps to: POST /airtime/cash/{reference}
 *
 * Partial completion: if the amount was split into multiple shares
 * and some fail, only the successfully-shared portion is reflected
 * in the returned amount — always credit the caller based on the
 * amount THIS response reports, not the amount originally requested.
 *
 * @param {string} reference - the reference returned by initiateAirtimeToCash()
 * @param {string} otp       - the OTP sent to the sender's phone
 */
async function completeAirtimeToCash(reference, otp) {
  return callProvider(
    {
      method: 'POST',
      url: `/airtime/cash/${encodeURIComponent(reference)}`,
      data: { otp }
    },
    'completeAirtimeToCash'
  );
}

/**
 * Resend the OTP for a pending Airtime-to-Cash request.
 * Maps to: POST /airtime/cash/{reference}/resend-otp
 *
 * @param {string} reference - the reference returned by initiateAirtimeToCash()
 */
async function resendAirtimeToCashOtp(reference) {
  return callProvider(
    {
      method: 'POST',
      url: `/airtime/cash/${encodeURIComponent(reference)}/resend-otp`,
      data: {}
    },
    'resendAirtimeToCashOtp'
  );
}
