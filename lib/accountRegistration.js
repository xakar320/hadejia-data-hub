...
async function createAccount({ fullName, email, phone, password, pinHash, referredBy = null }) {
  ...
  try {
    ...
    // created (confirmed by a users_pkey duplicate-key error on a
    // plain INSERT here). UPSERT on id overwrites that stub with our
    // real registration data instead of colliding with it, and still
    // behaves correctly as a plain insert if no such trigger exists.
