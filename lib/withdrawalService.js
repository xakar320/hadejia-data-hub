 ...
 */
/**
 * NOTE: SecureWaveNG's automatic account-name verification
 * (/customer_withdrawals/validate-account-name) lives in the same
 * "Customer Withdrawals" folder as the withdraw endpoint itself,
 * which is not enabled for this merchant — so it's not usable here.
 * The customer types their own account name instead; since payouts
 * are manual anyway, the admin can sanity-check the name against the
 * account number in their own banking app before paying.
 */

/**
 * Save the bank account a user withdraws to. Stored in OUR OWN
 * customer_bank_info table (NOT on SecureWaveNG — their
 * customer_withdrawals/bank-info endpoint is gated behind the same
 * merchant permission the withdraw endpoint needs, which isn't
...
async function saveBankInfo({ userId, bankName, accountName, bankCode, accountNumber }) {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .upsert(
      { user_id: userId, bank_name: bankName, account_name: accountName, bank_code: bankCode, account_number: accountNumber, updated_at: new Date().toISOString() },
      { onConflict: 'user_id' }
    )
    .select()
    .single();
  ...
  if (error) {
    throw new WithdrawalError(`Failed to save bank info: ${error.message}`, { statusCode: 500 });
  }

  return { bankName: data.bank_name, accountNumber: data.account_number, accountName: data.account_name, bankCode: data.bank_code };
}

/**
 * Read back the bank account currently on file for this user, or
 * null if none has been saved yet. Reads from our own table (see
 * saveBankInfo note above).
 */
