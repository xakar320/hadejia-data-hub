...
/**
 * Picks the price a specific customer should pay for a plan —
 * reseller_price if they're an approved reseller AND that plan has
 * one configured, otherwise the normal selling_price. Falls back
 * safely to selling_price if reseller_price is null (not every plan
 * needs a special reseller rate).
 */
function effectivePrice(plan, isReseller) {
  if (isReseller && plan.reseller_price !== null && plan.reseller_price !== undefined) {
    return Number(plan.reseller_price);
  }
  return Number(plan.selling_price);
}

async function resolveDataOrder(body, isReseller) {
  ...
  if (apiProvider === 'supercheapdata') {
    ...
    return {
      amount: effectivePrice(plan, isReseller),
...
async function resolveVoiceOrder(body, isReseller) {
  validateRequiredFields(body, ['planId', 'phone']);
...
/**
 * Splits a comma-separated string or array of recipients into a
 * deduped list of validated, normalized Nigerian phone numbers.
 * Reuses validatePhone (throws ValidationError on any bad entry) so
 * an SMS never goes out to a malformed number.
 */
function parseSmsRecipients(raw) {
  const list = Array.isArray(raw) ? raw : String(raw || '').split(',');
  const cleaned = list.map((r) => String(r).trim()).filter(Boolean);

  if (cleaned.length === 0) {
    throw new ValidationError('recipients must contain at least one phone number', {
      details: [{ field: 'recipients', message: 'recipients must contain at least one phone number' }]
    });
  }
  if (cleaned.length > 1000) {
    throw new ValidationError('recipients cannot exceed 1000 numbers per request', {
      details: [{ field: 'recipients', message: 'Maximum 1000 recipients per request' }]
    });
  }

  const normalized = cleaned.map((r) => validatePhone(r, 'recipients'));
  return [...new Set(normalized)];
}

/**
 * SMS pricing is unit-based rather than plan-based (per AutosyncNG
 * docs: units = ceil(message.length / 160) x recipients.length,
 * billed per unit), so this reads a single active row from
 * sms_plans (rather than filtering by network/id like the other
 * *_plans tables) for price_per_unit / cost_per_unit / a default
 * sender_id, and computes amount/costPrice here rather than off a
 * fixed selling_price.
 *
 * Required table (create once in Supabase if it doesn't exist yet):
 *   sms_plans (
 *     id uuid primary key default gen_random_uuid(),
 *     status text not null default 'active',
 *     sender_id text,                 -- default sender ID, used if the caller doesn't pass one
 *     price_per_unit numeric not null,        -- what the customer pays per unit
 *     cost_per_unit numeric,                  -- what AutosyncNG charges per unit (for profit tracking; falls back to price_per_unit)
 *     reseller_price_per_unit numeric         -- optional discounted per-unit rate for resellers
 *   )
 * Insert exactly one 'active' row to configure pricing.
 */
async function resolveSmsOrder(body, isReseller) {
  validateRequiredFields(body, ['recipients', 'message']);
  ...
  if (message.length > 1000) {
    throw new ValidationError('message must be at most 1000 characters', {
      details: [{ field: 'message', message: 'message must be at most 1000 characters' }]
    });
  }

  const recipients = parseSmsRecipients(body.recipients);

  const plan = await fetchActivePlan('sms_plans', {}, 'SMS sending is not currently available');

  const pricePerUnit =
    isReseller && plan.reseller_price_per_unit !== null && plan.reseller_price_per_unit !== undefined
      ? Number(plan.reseller_price_per_unit)
      : Number(plan.price_per_unit);

  const costPerUnit =
    plan.cost_per_unit !== null && plan.cost_per_unit !== undefined ? Number(plan.cost_per_unit) : pricePerUnit;

  const segments = Math.ceil(message.length / 160);
  const units = segments * recipients.length;

  const amount = Math.round(units * pricePerUnit * 100) / 100;
  const costPrice = Math.round(units * costPerUnit * 100) / 100;

  const senderId = (body.senderId ? String(body.senderId).trim() : '') || plan.sender_id;
  if (!senderId) {
    throw new ValidationError('senderId is required (no default sender_id configured)', {
      details: [{ field: 'senderId', message: 'senderId is required (no default sender_id configured)' }]
    });
  }
  if (senderId.length > 20) {
    throw new ValidationError('senderId must be at most 20 characters', {
      details: [{ field: 'senderId', message: 'senderId must be at most 20 characters' }]
    });
  }

  const recipientDisplay =
    recipients.length > 1 ? `${recipients[0]} +${recipients.length - 1} more` : recipients[0];
  ...
  return {
    ...
    planId: null,
    requestPayload: { recipients, message, senderId, units, segments },
    callProvider: (reference) => autosync.purchaseSms({ recipients, message, senderId, reference })
  };
}

async function resolveCableOrder(body, isReseller) {
  ...
  if (cableType === 'change') {
    ...
    const amount = effectivePrice(plan, isReseller);
...
async function executeOrder({ userId, type, pin, requestRef, ...orderFields }) {
  ...
  const { data: userRow } = await supabaseAdmin.from('users').select('is_reseller').eq('id', userId).maybeSingle();
  const isReseller = !!(userRow && userRow.is_reseller);
  ...
  const order = await resolveOrder(orderFields, isReseller);
  ...
  // NEVER echo providerResult.message to the customer — it can contain
  // internal provider names/details (e.g. "SuperCheapData request
  // timed out", "AutosyncNG: Invalid product code") that shouldn't be
  // customer-facing. The real reason is still preserved server-side,
  // in transactions.reason above and in the logs, for admin/debugging.
  throw new OrderFailedError('We could not complete this purchase right now. The amount has been refunded to your wallet.', {
