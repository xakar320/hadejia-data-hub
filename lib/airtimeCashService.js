...
async function initiateRequest({ userId, productId, phone, amount, sharePin }) {
  ...
  const { data: row, error: insertError } = await supabaseAdmin
    ...
    .insert({
      ...
      phone: normalizedPhone,
      receiving_number: plan.receiving_number || null,
      amount: numericAmount,
...
async function completeRequest({ userId, reference, otp }) {
  ...
  // Log to the shared transactions table (not just wallet_history) so
  // this shows up in the customer/admin transaction history view,
  // which reads from `transactions`, not `wallet_history`. The
  // history page shows `network` and `recipient` together as one
  // line, so pack "sender → receiving line" into `recipient` here —
  // that's the one piece admin wanted visible that wasn't tracked
  // anywhere before (row.receiving_number, snapshotted at request
  // time from the plan's admin-set value).
  await supabaseAdmin.from('transactions').insert({
    user_id: userId,
    type: 'airtime_to_cash',
    network: row.network,
    recipient: `${row.phone} → ${row.receiving_number || 'your line'}`,
    amount: creditAmount,
    status: 'success',
    wallet_history_id: credit.walletHistoryId,
    request_payload: {
      airtimeAmount: actualAmount,
