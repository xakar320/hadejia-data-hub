...
function requireAuth(handler) {
  return async function wrappedHandler(req, res) {
    ...
    // Only token verification is guarded above. Anything the wrapped
    // handler itself throws (WalletError, TransactionError, a Supabase
    // query error, etc.) must propagate to the caller's own error
    // handling (e.g. response.js's withErrorHandling) rather than being
    // caught here and mislabeled as an authentication failure.
    req.user = user;
...
/**
 * Verify a customer's transaction PIN against their stored pin_hash.
 * The PIN is hashed here (SHA-256) and compared server-side — the
 * client sends the plaintext PIN over HTTPS (never the hash itself,
 * which would otherwise be a directly-replayable credential).
 *
 * Throws AuthError:
 *   - PIN_NOT_SET (400) if the user has never set a PIN yet
 *   - INVALID_PIN (401) if it doesn't match
 *
 * @param {string} userId
 * @param {string} pin - plaintext 4-6 digit PIN
 */
async function verifyPin(userId, pin) {
  ...
  if (error || !data) {
    throw new AuthError('Unable to verify PIN', { statusCode: 500, code: 'PIN_VERIFY_FAILED' });
  }

  if (!data.pin_hash) {
    throw new AuthError('You have not set a transaction PIN yet. Please set one in your Profile.', {
      statusCode: 400,
      code: 'PIN_NOT_SET'
    });
  }
  ...
  if (hash !== data.pin_hash) {
    throw new AuthError('Incorrect transaction PIN', { statusCode: 401, code: 'INVALID_PIN' });
  }
