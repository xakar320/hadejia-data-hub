...
let offset = 0;
const PAGE_SIZE = 15;
let filters = { type: '', status: '', search: '' };
...
async function loadTransactions(reset) {
    ...
    try {
        ...
        if (filters.search) {
            // Comma would break the .or() filter syntax below — strip it defensively.
            const term = filters.search.replace(/,/g, '');
            query = query.or(`recipient.ilike.%${term}%,provider_reference.ilike.%${term}%`);
        }
...
function renderTxItem(tx) {
    ...
    const sign = ['wallet_funding', 'airtime_to_cash'].includes(tx.type) ? '+' : '−';
