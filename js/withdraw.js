...
async function init() {
    ...
    $('#account-name-input').addEventListener('input', onBankFieldsChanged);
...
function onBankFieldsChanged() {
    ...
    if (selectedBank && /^\d{10}$/.test(accountNumber) && accountName.length >= 3) {
        btn.disabled = false;
        btn.textContent = 'Save this account';
    } else {
        btn.disabled = true;
        btn.textContent = 'Fill in all fields above';
    }
}
...
$('#withdraw-btn').addEventListener('click', async () => {
    ...
    try {
        ...
        showResultModal({
            ...
            message: `Your request to withdraw ₦${money(result.amount)} to ${result.bankName} — ${result.accountNumber} has been received (₦${money(result.fee)} fee, ₦${money(result.totalDebited)} debited). It will be paid out manually — this can take a few hours.`
        });
