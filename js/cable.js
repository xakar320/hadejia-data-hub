...
function displayPrice(pkg) {
    if (isReseller && pkg.reseller_price !== null && pkg.reseller_price !== undefined) {
        return Number(pkg.reseller_price);
    }
    return Number(pkg.selling_price);
}
...
function renderPackageList() {
    ...
    packages.forEach(pkg => {
        ...
        row.innerHTML = `
            ...
            <div class="price mono">₦${money(displayPrice(pkg))}</div>
        `;
...
function updateBuyButton() {
    ...
    if (mode === 'renew') {
        ...
        if (selectedPackage) {
            btn.disabled = false;
            btn.textContent = `Subscribe — ₦${money(displayPrice(selectedPackage))}`;
        } else {
            btn.disabled = true;
