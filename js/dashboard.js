...
// ===============================
// WITHDRAW FEATURE TOGGLE
// ===============================
// Reads the admin_settings row the Admin panel's "Withdraw feature"
// card writes to (setting_key = 'withdraw_enabled', setting_value =
// { enabled }). Hides the Withdraw tile on the dashboard when off —
// withdraw.html itself also re-checks this so a saved bookmark can't
// bypass it.

async function checkWithdrawFeatureFlag() {
    try {
        const { data, error } = await client
            .from('admin_settings')
            .select('setting_value')
        ...
        if (error || !data) return; // no row yet = treat as enabled (fail open, matches default)

        const { enabled } = data.setting_value || {};
        const tile = document.getElementById('withdrawTile');

        if (enabled === false && tile) {
            tile.style.display = 'none';
        }
    } catch (err) {
        console.error('Failed to load withdraw feature flag:', err);
    }
}
